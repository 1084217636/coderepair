...
The Anchored Branch extension reuses DeerFlow Thread, Run, Checkpoint, Agent, Tool, Sandbox, StreamBridge, and SSE. Its custom domain is limited to `deerflow.anchored_branch`, `/api/anchored-branches`, and the frontend selection panel. A branch is a Child Thread with `parent_thread_id` metadata; do not create a parallel branch-message store or reimplement the run/stream lifecycle. `BranchContextBuilder` must hard-preserve the selected Anchor and current question while bounding summary, history, and code context. `Apply to Main` writes a structured `anchored_branch_decision` to the main Thread and is not permission to modify code directly; the next main Run must let the Agent inspect, change, and test through the existing Tool/Sandbox path.

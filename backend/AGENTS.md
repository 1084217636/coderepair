...
The Anchored Branch extension reuses DeerFlow Thread, Run, Checkpoint, Agent, Tool, Sandbox, StreamBridge, and SSE. Its custom domain is limited to `deerflow.anchored_branch`, `/api/anchored-branches`, and the frontend selection panel. A branch is a Child Thread with `parent_thread_id` metadata; do not create a parallel branch-message store or reimplement the run/stream lifecycle. Creation must validate the assistant message ID, selected text and offsets. `BranchContextBuilder` combines Main Task Summary, Anchor, Relevant Main Context and Branch History under a token budget. Branch runs and close operations must never mutate the Main Thread. Do not add Decision Capsule, Context Ledger, Accept/Edit/Reject, Stale, Conflict, Supersede, or implicit Apply-to-Main behavior.
...
Real-model evaluation is intentionally limited to two suites. `deerflow.code_change.agent_evaluation` contains 12 auto-verifiable repositories and must execute the real Requirement → Retrieval → Agent → Tool → Patch → Test path. `deerflow.anchored_branch.benchmark` contains 12 cases and invokes the same configured model for Full History, Anchor Only, and Anchored Context. Keep these separate from the 20-case external-patch gate, report the task count with every percentage, and never hand-edit evaluation numbers. The patch submit Tool runs `git apply --check`; when that deterministic validation fails, `generate_patch_with_agent` may start one fresh graph with the exact failure feedback, but must not retry more than once.

Code-change tasks declare `patch_mode=external|agent`. Agent mode is a real Worker stage: the task records its model name and task-level Agent correlation IDs (`agent_thread_id` / `agent_run_id`), enters `GENERATING_PATCH`, calls the restricted `deerflow.code_change.agent_patch` graph (read/search/typed patch submission only), and then rejoins the deterministic validation path. Those IDs are Task metadata passed into graph invocation; this extension does not create persisted Gateway Thread/Run records and does not configure a graph checkpointer. The Agent must inspect the pinned-commit workspace, never the mutable source checkout. HTTP project creation accepts only server-owned test profiles (`python-pytest`, `go-test`, `frontend-check` by default). The feature is disabled unless `DEER_FLOW_CODE_CHANGE_ENABLED=true`; `/api/code-change/worker/run-once` additionally requires the dedicated `DEER_FLOW_CODE_CHANGE_WORKER_TOKEN` via `X-Code-Change-Worker-Token`, scoped to that exact path. Test subprocesses run with a scrubbed environment and, on POSIX, in a new process group that is killed as a unit on timeout.

Code retrieval is deliberately lightweight and explainable: `context_retriever.py` chunks indexed source, combines lexical, language-specific symbol, and optional semantic scores, then `build_retrieval_context` packs ranked chunks under a token budget. Semantic retrieval is enabled only through `CODE_CHANGE_EMBEDDING_*` and must fall back to lexical + symbol when unavailable. Do not add a vector database, Cross Encoder, GraphRAG, or complex reranker. Worker-selected contexts must enter the Agent's initial prompt, and a Branch linked to an owner-scoped Code Change project must reuse this same retrieval path for each code question.
